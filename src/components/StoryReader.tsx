...
import { ArrowLeft, BookOpen, Volume2, Award, Target, Sparkles, ChevronLeft, ChevronRight, Play } from "lucide-react";
...
export function StoryReader({ 
  ...
  const handleWordComplete = (wordId: string, difficulty: number) => {
    ...
    const justCompletedTodayWords = todayWordsInStoryIds.length > 0 && 
                                     todayWordsInStoryIds.every(id => completedWordIds.includes(id)) &&
                                     !Object.keys(wordProgress).every(id => todayWordsInStoryIds.includes(id) ? completedWordIds.includes(id) : true);
    ...
    if (justCompletedTodayWords) {
      setTimeout(() => {
        toast.success(`Great work! You've reviewed all ${todayWordsInStory.length} words scheduled for today! 🎉`);
      }, 300);
    }
  };
  ...
  const todayWordsInStory = story.words.filter(w => w.isVocabulary && todayWordIds.includes(w.id));
  const todayWordsCompleted = todayWordsInStory.every(w => Object.keys(wordProgress).includes(w.id));
  const allTodayWordsReviewed = todayWordsInStory.length > 0 && todayWordsCompleted;
          ...
          <div className="flex items-start gap-4 mb-4">
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                {incompleteStories && incompleteStories.length > 1 && currentStoryIndex !== undefined && currentStoryIndex >= 0 && (
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground mt-0.5">
                        You have {(incompleteStories?.length || 1) - (currentStoryIndex || 0) - 1} more {((incompleteStories?.length || 1) - (currentStoryIndex || 0) - 1) === 1 ? 'story' : 'stories'} to complete
                      </p>
                    </div>
                  </div>
                  <Button onClick={onNext} className="bg-primary hover:bg-primary/90 shrink-0">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-green-700 mt-0.5">
                      You've reviewed all {todayWordsInStory.length} word{todayWordsInStory.length !== 1 ? 's' : ''} scheduled for today!
                    </p>
                  </div>
                </div>
              </div>
                ...
                {progress === 100 ? (
                  ...
                  <Card className="border border-green-600/40 bg-gradient-to-br from-green-50 via-emerald-50 to-green-50 overflow-hidden relative">
                    ...
                    <div className="p-4 relative">
                      ...
                      {/* Progress Stats */}
                      {(easyWords > 0 || goodWords > 0 || hardWords > 0 || againWords > 0) && (
                        <motion.div
                          initial={{ opacity: 0, y: 10 }}
                          animate={{ opacity: 1, y: 0 }}
                        ...
                        {(easyWords > 0 || goodWords > 0 || hardWords > 0 || againWords > 0) && (
                          <div className="flex flex-wrap gap-2 pt-1">
                            {easyWords > 0 && (
                              ...
                              >
                                <span className="text-xs text-green-700">{easyWords} easy</span>
                              </motion.div>
                            )}
                            {goodWords > 0 && (
                              <motion.div
                              ...
                              >
                                <span className="text-xs text-yellow-700">{goodWords} good</span>
                              </motion.div>
                            )}
                            {hardWords > 0 && (
                              <motion.div
                              ...
                              >
                                <span className="text-xs text-orange-700">{hardWords} hard</span>
                              </motion.div>
                            )}
                            {againWords > 0 && (
                              <motion.div
                              ...
                              >
                                <span className="text-xs text-red-700">{againWords} again</span>
                              </motion.div>
                            )}
                          </div>
                        )}
                  ...
                  <div className="p-5 flex gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground leading-relaxed">
                        Click any <span className="text-primary border-b-2 border-primary/60 px-1">highlighted word</span> to see its meaning and rate your knowledge. Your progress is saved automatically.
                      </p>
                      {todayWordsInStory.length > 0 && (
                        <div className="mt-3 pt-3 border-t border-primary/20">
                          <p className="text-sm text-primary flex items-center gap-2">
                            <Target className="w-4 h-4" />
                            <span><strong>{todayWordsInStory.length}</strong> word{todayWordsInStory.length !== 1 ? 's' : ''} scheduled for today</span>
                          </p>
                        </div>
                      )}
                    </div>
          ...
          <div className="space-y-6">
            <Card className="border-2">
              <div className="p-6">
                ...
                <div className="space-y-2">
                  {story.words.filter(w => w.isVocabulary).map((word) => {
                    const difficulty = wordProgress[word.id];
                    const isDueToday = todayWordIds.includes(word.id);
                    const status = 
                      difficulty === 3 ? { color: 'bg-green-100 border-green-300 text-green-700', icon: '✓' } :
                      difficulty === 2 ? { color: 'bg-yellow-100 border-yellow-300 text-yellow-700', icon: '○' } :
                      difficulty === 1 ? { color: 'bg-orange-100 border-orange-300 text-orange-700', icon: '!' } :
                      difficulty === 0 ? { color: 'bg-red-100 border-red-300 text-red-700', icon: '✕' } :
                      { color: 'bg-gray-50 border-gray-200 text-gray-600', icon: '·' };
                        ...
                        <div className="flex items-center justify-between gap-2">
                          <span className="text-sm flex items-center gap-1.5">
                            ...
                            {isDueToday && !difficulty && (
                              <span className="text-xs bg-primary text-white px-1.5 py-0.5 rounded">Today</span>
                            )}
                          </span>
                ...
                <Card className="border-2 border-green-200 bg-gradient-to-br from-green-50 to-emerald-50">
                  <div className="p-6 text-center">
                    ...
                    <p className="text-muted-foreground text-sm">
                      All {todayWordsInStory.length} scheduled word{todayWordsInStory.length !== 1 ? 's' : ''} reviewed!
                    </p>
                  </div>
                </Card>
              </motion.div>
                ...
                <Card className="border-2 border-green-200 bg-gradient-to-br from-green-50 to-emerald-50">
                  <div className="p-6 text-center">
                    ...
                    <p className="text-green-600 text-sm mt-2">
                      Great job learning all {story.words.filter(w => w.isVocabulary).length} words!
                    </p>
                  </div>
                </Card>
              </motion.div>
              ...
              <div className="p-6">
                <div className="flex items-center justify-between gap-4">
                  ...
                  <div className="hidden sm:flex items-center gap-2 text-sm text-muted-foreground">
                    <span>Story {currentStoryIndex + 1} of {incompleteStories.length}</span>
                  </div>
