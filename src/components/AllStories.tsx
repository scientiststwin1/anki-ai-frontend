...
import { ArrowLeft, BookOpen, Calendar, Sparkles, Target, TrendingUp } from "lucide-react";
...
export function AllStories({ stories, onBack, onSelectStory }: AllStoriesProps) {
  const completedCount = stories.filter(s => s.isComplete).length;
  const inProgressCount = stories.filter(s => !s.isComplete && s.completedWordIds.length > 0).length;
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            {stories.length > 0 && (
              <div className="flex gap-2">
                ...
                {inProgressCount > 0 && (
                  <Badge variant="secondary" className="border-2 border-primary/30">
                  ...
                  >
                    ...
                    <div className="p-6">
                      <div className="flex items-start justify-between gap-4 mb-4">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            <div className={`${story.isComplete ? 'bg-gradient-to-br from-green-600 to-emerald-600' : 'bg-primary/10'} rounded-lg p-1.5`}>
                              <BookOpen className={`w-4 h-4 ${story.isComplete ? 'text-white' : 'text-primary'}`} />
                            </div>
                      ...
                      <div className="flex flex-wrap items-center gap-2.5 text-sm">
                        ...
                        {!story.isComplete && story.completedWordIds.length > 0 && (
                          <Badge className="bg-gradient-to-r from-amber-500 to-orange-500 text-white border-0 shadow-sm">
                            <Target className="w-3 h-3 mr-1" />
                            In Progress
                          </Badge>
                        )}
                        <Badge variant="secondary" className="capitalize border-2 border-primary/20">
                          {story.difficulty}
                        </Badge>
                        <div className="flex items-center gap-1.5 text-muted-foreground">
                          <Calendar className="w-3.5 h-3.5" />
                          <span>{story.words.filter(w => w.isVocabulary).length} words</span>
                        </div>
                        {!story.isComplete && story.completedWordIds.length > 0 && (
                          <div className="flex items-center gap-1.5">
                            <div className="flex-1 h-1.5 bg-muted rounded-full overflow-hidden">
                              <motion.div 
                                ...
                                initial={{ width: 0 }}
                                animate={{ width: `${(story.completedWordIds.length / story.words.filter(w => w.isVocabulary).length) * 100}%` }}
                                transition={{ duration: 0.5, delay: index * 0.05 }}
                              />
                            </div>
                            <span className="text-xs text-primary min-w-[45px]">
                              {story.completedWordIds.length}/{story.words.filter(w => w.isVocabulary).length}
                            </span>
                          </div>
                        )}
                      </div>
