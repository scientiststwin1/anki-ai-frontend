...
export function WordList({ onRemoveWord }: WordListProps) {
    ...
    <div>
      ...
      {getAllCardsData?.data?.length === 0 ? (
        ...
        <div className="space-y-2.5">
          {getAllCardsData?.data.map((card: any, index: number) => (
            <motion.div
              key={card.id}
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex items-start gap-3 flex-1">
                        ...
                        <div className="flex-1 min-w-0">
                          <h4 className="mb-1">{card.word}</h4>
                          {card.note && (
                      ...
                      <Button
                        ...
                        size="icon"
                        onClick={() => {
                          onRemoveWord(card.id);
                          toast.success(`"${card.word}" removed`);
                        }}
